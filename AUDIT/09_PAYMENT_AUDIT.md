...
2. **Integer Minor Units & Money Safety:**
   - All monetary calculations are performed in integer minor units (cents) via the `Money` class in `@caribbean/Payments`. Floating-point arithmetic is banned across financial paths.
...
3. **Store Policy Engine (`PaymentPolicyEngine`):**
   ...
   - Physical goods, event tickets, and web transactions route through Payments Wallet, Stripe Cards, and PayPal.

---

## 2. PSP Adapter Status

- `StripeAdapter` and `PayPalAdapter` are defined with proper TypeScript interfaces (`PSPAdapter`) in `@caribbean/Payments/src/index.ts`, with clear exception stubs.
