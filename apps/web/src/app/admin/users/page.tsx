    ...
    <div className="min-h-screen bg-transparent text-brand-sandstone p-6 max-w-6xl mx-auto space-y-6">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 text-xs">
          {page > 1 && (
            <Link href={`?q=${query}&page=${page - 1}`} className="px-3 py-1.5 bg-brand-dusk text-slate-200 rounded-lg">← Prev</Link>
          )}
          <span className="text-brand-sandstone/60">Page {page} of {totalPages}</span>
          {page < totalPages && (
            <Link href={`?q=${query}&page=${page + 1}`} className="px-3 py-1.5 bg-brand-dusk text-slate-200 rounded-lg">Next →</Link>
          )}
        </div>
      )}
    </div>
