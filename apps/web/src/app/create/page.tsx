...
export default async function CreateHubPage() {
  ...
  return (
    <div className="min-h-screen bg-[#090D16] text-brand-sandstone p-4 md:p-6 max-w-6xl mx-auto space-y-10">
      {/* Top Header */}
      <div className="border-b border-slate-800 pb-6 space-y-2">
        <div className="flex items-center gap-2">
          <Sparkles className="w-7 h-7 text-brand-caribbeanSea" />
          <h1 className="text-2xl md:text-3xl font-black text-brand-sandstone tracking-tight">
            Antilia Creation Hub
          </h1>
        </div>
        <p className="text-xs md:text-sm text-brand-sandstone/60 max-w-2xl">
      ...
      <section id="composer" className="space-y-3">
        <h2 className="text-xs font-black text-brand-sandstone/60 uppercase tracking-widest px-1">
      ...
      <section className="space-y-4 pt-4 border-t border-slate-800">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-extrabold text-brand-sandstone uppercase tracking-wider">
            All Ecosystem Creation Tools
          </h2>
          <span className="text-xs text-brand-sandstone/40">12 Categories Available</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {CREATE_CATEGORIES.map((cat) => (
            <Link
              key={cat.id}
              href={cat.href}
              className={`bg-gradient-to-br ${cat.color} bg-brand-dusk/80 border rounded-3xl p-5 flex flex-col justify-between hover:scale-[1.02] transition-all shadow-xl group`}
            >
