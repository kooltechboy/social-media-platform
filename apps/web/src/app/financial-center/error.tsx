...
export default function FinancialCenterError({
  error,
  reset,
}: {
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center p-6 text-brand-sandstone w-full">
      <div className="bg-brand-dusk/80 border border-slate-800 rounded-3xl p-8 max-w-md w-full text-center space-y-4 shadow-2xl">
        <div className="w-12 h-12 rounded-full bg-brand-sunriseCoral/10 text-brand-sunriseCoral flex items-center justify-center mx-auto">
          <Wallet className="w-6 h-6" />
        </div>
        <h2 className="text-xl font-bold text-white">Financial Ledger Safe Mode</h2>
        <p className="text-xs text-brand-sandstone/70 leading-relaxed">
          {error.message || 'Financial Center records are protected. An unexpected error occurred while loading ledger data.'}
        </p>
        <div className="pt-2 flex items-center justify-center gap-3">
          ...
          >
            <RefreshCw className="w-3.5 h-3.5" /> Reconnect Ledger
          </button>
        </div>
      </div>
    </div>
