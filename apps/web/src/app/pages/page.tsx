...
export default async function PagesDirectoryPage({
  ...
  if (supabase) {
    ...
    const [dbRes, myRes] = await Promise.all([
      query,
      user
        ? supabase
            .from('businesses')
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 xl:gap-8 items-start w-full">
      <div className="flex-1 min-w-0 space-y-8 w-full max-w-[820px] xl:max-w-[860px] mx-auto lg:mx-0 animate-fadeIn">
        {/* Top Header */}
        <div className="surface-header p-6 sm:p-8 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-6 border border-brand-sunriseCoral/30 shadow-xl">
        ...
        {allPages.length === 0 ? (
          <div className="surface-card rounded-3xl p-12 text-center space-y-4 max-w-xl mx-auto border border-white/10">
            ...
            <h3 className="text-lg font-black text-white">No pages found</h3>
            <p className="text-xs sm:text-sm text-brand-sandstone/80 leading-relaxed">
              No Caribbean pages match your current filter. Launch your official business or organization page today!
            </p>
          </div>
        ) : (
