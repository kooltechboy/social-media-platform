...
export default async function ProfilePage({
  ...
  const [countsResult, relationshipBatch, postsResult, recognition, friendsResult, pagesResult, communitiesResult, podcastsResult] = await Promise.all([
    supabase
      .from('profile_counts')
    ...
    supabase
      .from('businesses')
      .select('id, name, slug, avatar_url, category, is_verified, page_type, description')
      .eq('owner_id', profileData.id)
      .limit(12),
    supabase
      .from('community_members')
      .select('community_id, communities(id, name, slug, member_count, country_iso, description)')
      .eq('profile_id', profileData.id)
      .eq('membership_status', 'active')
      .limit(12),
    supabase
      .from('podcasts')
      .select('id, title, subtitle, slug, description, category, follower_count, cover_path')
      .eq('creator_id', profileData.id)
      .limit(12),
  ]);

  const userPages = pagesResult?.data ?? [];
  const userCommunities = ((communitiesResult?.data ?? []) as any[])
    .map((c: any) => c.communities)
    .filter(Boolean);
  const userPodcasts = (podcastsResult?.data ?? []) as any[];
    ...
    <div className="min-h-screen bg-transparent text-brand-sandstone pb-12">
      ...
      <main className="max-w-5xl 3xl:max-w-6xl mx-auto w-full py-2 sm:py-4 space-y-6">
        ...
        <nav className="flex border-b border-white/15 gap-8 text-sm md:text-base font-black overflow-x-auto scrollbar-none" aria-label="Profile navigation">
          ...
          {userPages.length > 0 && (
            <Link
              href={`/profile/${profileData.username}?tab=pages`}
              className={`pb-3.5 md:pb-4 border-b-2 transition-all flex items-center gap-2 min-h-[44px] md:min-h-[48px] whitespace-nowrap ${
                tab === 'pages'
            ...
            >
              <span>Pages</span>
              <span className="text-[10px] md:text-xs px-2 md:px-2.5 py-0.5 rounded-full bg-cyan-500/20 text-cyan-300 font-mono font-bold border border-cyan-500/30">
                {userPages.length}
              </span>
            </Link>
          )}
          {userCommunities.length > 0 && (
            <Link
              href={`/profile/${profileData.username}?tab=communities`}
              className={`pb-3.5 md:pb-4 border-b-2 transition-all flex items-center gap-2 min-h-[44px] md:min-h-[48px] whitespace-nowrap ${
                tab === 'communities'
                  ? 'border-brand-sunriseCoral text-brand-sunriseCoral font-black'
                  : 'border-transparent text-brand-sandstone/70 hover:text-white'
              }`}
            >
              <span>Communities</span>
              <span className="text-[10px] md:text-xs px-2 md:px-2.5 py-0.5 rounded-full bg-rose-500/20 text-rose-300 font-mono font-bold border border-rose-500/30">
          ...
          {userPodcasts.length > 0 && (
            <Link
              href={`/profile/${profileData.username}?tab=podcasts`}
              className={`pb-3.5 md:pb-4 border-b-2 transition-all flex items-center gap-2 min-h-[44px] md:min-h-[48px] whitespace-nowrap ${
                tab === 'podcasts'
            ...
            >
              <span>Podcasts</span>
              <span className="text-[10px] md:text-xs px-2 md:px-2.5 py-0.5 rounded-full bg-purple-500/20 text-purple-300 font-mono font-bold border border-purple-500/30">
                {userPodcasts.length}
              </span>
            </Link>
          )}
            ...
            {userPages.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {userPages.map((p: any) => (
                  <div
                    key={p.id}
                    className="surface-card rounded-2xl p-5 border border-white/10 bg-[#140C22]/80 hover:border-brand-caribbeanSea/40 transition-all space-y-3"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-2xl bg-brand-twilight border border-white/10 flex items-center justify-center overflow-hidden shrink-0">
                        {p.avatar_url ? (
                          <img src={p.avatar_url} alt={p.name} className="w-full h-full object-cover" />
                        ) : (
                          <Building2 className="w-6 h-6 text-brand-goldenHour" />
                        )}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          <h4 className="text-sm font-black text-white truncate">{p.name}</h4>
                          {p.is_verified && <BadgeCheck className="w-4 h-4 text-orange-400 shrink-0" />}
                        </div>
                        <p className="text-[11px] text-brand-sandstone/60 capitalize truncate">
                          {p.category || p.page_type || 'Universal Page'}
                        </p>
                      </div>
                    </div>
                    {p.description && (
                    ...
                    <div className="pt-2 border-t border-white/10 flex justify-end">
                      <Link
                        href={`/pages/${p.slug}`}
                        className="px-4 py-2 rounded-xl bg-brand-caribbeanSea text-slate-950 text-xs font-black hover:brightness-110 transition-all flex items-center gap-1.5 shadow-sm"
                      >
                        <span>View Page</span>
              ...
              <div className="surface-card rounded-3xl p-12 text-center space-y-4 border border-white/15 bg-[#140C22]/80 max-w-md mx-auto">
                <Building2 className="w-12 h-12 text-brand-caribbeanSea/60 mx-auto" />
                <div className="space-y-1">
                  <h3 className="text-base sm:text-lg font-black text-white">No Pages Managed</h3>
                  <p className="text-xs text-brand-sandstone/70 leading-relaxed">
                    {isOwnProfile
                      ? 'Launch an official Universal Page for your Caribbean brand, business, creator identity, or initiative.'
                      : `@${profileData.username} doesn't manage any public Pages.`}
                  </p>
                </div>
                {isOwnProfile && (
                  <Link
            ...
            {userCommunities.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {userCommunities.map((c: any) => (
                  <div
                    key={c.id}
                    className="surface-card rounded-2xl p-5 border border-white/10 bg-[#140C22]/80 hover:border-brand-sunriseCoral/40 transition-all space-y-3"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-2xl bg-brand-twilight border border-white/10 flex items-center justify-center text-xl shrink-0">
                        🌴
                      </div>
                      <div className="min-w-0 flex-1">
                        <h4 className="text-sm font-black text-white truncate">{c.name}</h4>
                        <p className="text-[11px] text-brand-sunriseCoral font-bold flex items-center gap-1">
                    ...
                    <div className="pt-2 border-t border-white/10 flex justify-end">
                      <Link
                        ...
                        className="px-4 py-2 rounded-xl bg-brand-sunriseCoral text-slate-950 text-xs font-black hover:brightness-110 transition-all flex items-center gap-1.5 shadow-sm"
                      >
                        <span>Visit Hub</span>
                        <ArrowRight className="w-3.5 h-3.5" />
                      </Link>
              ...
              <div className="surface-card rounded-3xl p-12 text-center space-y-4 border border-white/15 bg-[#140C22]/80 max-w-md mx-auto">
                ...
                <div className="space-y-1">
                  <h3 className="text-base sm:text-lg font-black text-white">No Communities Joined</h3>
                  <p className="text-xs text-brand-sandstone/70 leading-relaxed">
                    {isOwnProfile
                      ? 'Explore vibrant island communities and global diaspora hubs across our network.'
                      : `@${profileData.username} hasn't joined any public community hubs yet.`}
                  </p>
                </div>
                {isOwnProfile && (
                  <Link
        ...
        {/* TAB: PODCASTS CONTENT */}
        {tab === 'podcasts' && (
          <section className="space-y-6 animate-fadeIn">
            {userPodcasts.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {userPodcasts.map((pod: any) => (
                  <div
                    key={pod.id}
                    className="surface-card rounded-2xl p-5 border border-white/10 bg-[#140C22]/80 hover:border-purple-500/40 transition-all space-y-3"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-2xl bg-purple-600/20 text-purple-400 border border-purple-500/30 flex items-center justify-center text-xl shrink-0">
                        🎙️
                      </div>
                      <div className="min-w-0 flex-1">
                        <span className="text-[10px] font-black uppercase text-purple-400 block">{pod.category || 'Culture & Talk'}</span>
                        <h4 className="text-sm font-black text-white truncate">{pod.title}</h4>
                        <p className="text-[11px] text-brand-sandstone/70">
                          {pod.follower_count || 0} Subscribers
                        </p>
                      </div>
                    </div>
                    {pod.description && (
                    ...
                    <div className="pt-2 border-t border-white/10 flex justify-end">
                      <Link
                        ...
                        className="px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-black transition-all flex items-center gap-1.5 shadow-sm"
                      >
              ...
              <div className="surface-card rounded-3xl p-12 text-center space-y-4 border border-white/15 bg-[#140C22]/80 max-w-md mx-auto">
                <Mic className="w-12 h-12 text-purple-400/60 mx-auto" />
                <div className="space-y-1">
                  <h3 className="text-base sm:text-lg font-black text-white">No Podcasts Published</h3>
                  <p className="text-xs text-brand-sandstone/70 leading-relaxed">
                    {isOwnProfile
                      ? 'Launch and broadcast original Caribbean audio shows and video podcasts directly from Creator Studio.'
                      : `@${profileData.username} hasn't published any podcast series yet.`}
                  </p>
                </div>
                {isOwnProfile && (
                  <Link
                    href="/creator-studio?tab=podcasts"
                    className="inline-flex items-center gap-2 text-xs font-black px-5 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white shadow-md transition-all"
                  >
                    <span>Launch Podcast Show</span>
                  </Link>
                )}
              </div>
            )}
