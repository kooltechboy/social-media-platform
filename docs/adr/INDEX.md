# ADR Index — CARIBBEAN ONE

Architectural Decision Records. Format: Status / Context / Decision / Consequences. New ADRs are appended; superseded ADRs are marked, never deleted.

| ADR | Title | Status |
| :--- | :--- | :--- |
| [ADR-001](ADR-001-monorepo.md) | pnpm + Turborepo monorepo | Accepted |
| [ADR-002](ADR-002-postgres-supabase.md) | PostgreSQL via Supabase as primary datastore | Accepted |
| [ADR-003](ADR-003-modular-monolith.md) | Modular monolith with event-driven extraction boundaries | Accepted |
| [ADR-004](ADR-004-mobile-architecture.md) | Expo React Native universal mobile app | Accepted |
| [ADR-005](ADR-005-media-storage.md) | Cloudflare R2/Stream object storage + CDN for media | Accepted |
| [ADR-006](ADR-006-live-streaming.md) | Managed live ingestion (never through app server) | Accepted |
| [ADR-007](ADR-007-podcast-architecture.md) | Podcast hosting with RSS distribution | Accepted |
| [ADR-008](ADR-008-spotpay.md) | SpotPay unified payment orchestration layer | Accepted |
| [ADR-009](ADR-009-financial-ledger.md) | Immutable double-entry ledger for all money movement | Accepted |
| [ADR-010](ADR-010-search.md) | Search behind a port; Postgres FTS now, dedicated engine later | Accepted |
| [ADR-011](ADR-011-ai-architecture.md) | Provider-agnostic AI via OpenRouter (CaribAI) | Accepted |
| [ADR-012](ADR-012-feed-architecture.md) | Multi-mode feed with ranking introduced behind flags | Accepted |
| [ADR-013](ADR-013-trust-safety.md) | Trust & Safety platform with human-in-the-loop | Accepted |
