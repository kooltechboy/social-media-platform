...
import { getClampedAspectRatio, type ClampedAspectRatioResult } from '../../packages/media/src/media-utils';
...
describe('Media Aspect Ratios & Design Tokens', () => {
  it('exports canonical TUKUBI aspect ratios and recommended pixel dimensions', () => {
    ...
    expect(ASPECT_RATIOS.cover).toBe('16 / 5');
    ...
    expect(RECOMMENDED_MEDIA_DIMENSIONS.camera).toEqual({ width: 1440, height: 1080 });
    expect(RECOMMENDED_MEDIA_DIMENSIONS.cameraPortrait).toEqual({ width: 1080, height: 1440 });
    expect(RECOMMENDED_MEDIA_DIMENSIONS.reels).toEqual({ width: 1080, height: 1920 });
    expect(RECOMMENDED_MEDIA_DIMENSIONS.avatar).toEqual({ width: 512, height: 512 });
    expect(RECOMMENDED_MEDIA_DIMENSIONS.cover).toEqual({ width: 1920, height: 600 });
  });

  it('correctly clamps aspect ratio between 16:9 and 4:5', () => {
    // 4:3 (1.33) is within bounds (0.8 to 1.777)
    const standardPhoto: ClampedAspectRatioResult = getClampedAspectRatio(4 / 3);
    ...
    const tallStory: ClampedAspectRatioResult = getClampedAspectRatio(9 / 16);
    ...
    const ultraWide: ClampedAspectRatioResult = getClampedAspectRatio(21 / 9);
